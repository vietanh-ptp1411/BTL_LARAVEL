...
<body>
    <div class="container">
        ...
        <div class="invoice-header">
            ...
            <p>Mã Hóa Đơn: {{$importbill->MaHDN}}</p>
            <p>Ngày: {{$importbill->ImpDate}}</p>
        </div>
        <div class="customer-info">
            <h2>Thông Tin Nhà Cung Cấp</h2>
            <p>Tên Nhà Cung Cấp: {{$importbill->supplier->SupName ?? 'N/A'}}</p>
            <p>Địa Chỉ: {{$importbill->supplier->SupAddress ?? 'N/A'}}</p>
            <p>Số điện thoại: {{$importbill->supplier->SupPhone ?? 'N/A'}}</p>
        </div>
        <div>
        ...
        <table class="table table-striped table-bordered bootstrap-datatable datatable">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="text-align: center;">{{$importbill->product->ProName ?? 'N/A'}}</td>          
                    <td style="text-align: center;"> {{$importbill->Quantity}}</td>        
                    <td style="text-align: center;">{{number_format($importbill->ImpPrice)}}đ</td>            
                    <td style="text-align: center;">{{number_format($importbill->Quantity * $importbill->ImpPrice) }} đ</td>            
                </td>
            </tbody>  
            {{-- @php $i = 1; @endphp
        ...
        <div style="float: right;margin-right:30px">
            <p><span style="color: blue; margin-right:15px">Tổng tiền:</span>{{number_format($importbill->Quantity * $importbill->ImpPrice) }} đ</p>          
        </div>
    </div>
</body>
</html>
